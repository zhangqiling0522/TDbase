/*
 * modified from the RTree implementation
 * at https://github.com/nushoin/RTree to support
...
/// ELEMTYPE Type of element that allows fractional and large values such as float or double, for use in volume calcs
///
...
protected:
  ...
  ELEMTYPE RectSphericalVolume(Rect* a_rect);
  ELEMTYPE RectVolume(Rect* a_rect);
  ELEMTYPE CalcRectVolume(Rect* a_rect);
  ...
  ELEMTYPE m_unitSphereVolume;                 ///< Unit sphere constant for required number of dimensions
};
...
RTREE_QUAL::RTree()
{
  ...
  m_root = AllocNode();
  m_root->m_level = 0;
  m_unitSphereVolume = (ELEMTYPE)UNIT_SPHERE_VOLUMES[NUMDIMS];
}
...
RTREE_TEMPLATE
ELEMTYPE RTREE_QUAL::RectVolume(Rect* a_rect)
{
  ASSERT(a_rect);
...
RTREE_TEMPLATE
ELEMTYPE RTREE_QUAL::RectSphericalVolume(Rect* a_rect)
{
  ASSERT(a_rect);
  ...
  for(int index=0; index < NUMDIMS; ++index) 
  {
    ELEMTYPE halfExtent = ((ELEMTYPE)a_rect->m_max[index] - (ELEMTYPE)a_rect->m_min[index]) * 0.5f;
  ...
  {
    return (ELEMTYPE)(pow(radius, NUMDIMS) * m_unitSphereVolume);
  }
}
...
RTREE_TEMPLATE
ELEMTYPE RTREE_QUAL::CalcRectVolume(Rect* a_rect)
{
       ...
       && (a_parVars->m_count[1] < (a_parVars->m_total - a_parVars->m_minFill)))
  {
    ...
    for(int index=0; index<a_parVars->m_total; ++index)
    {
      if(PartitionVars::NOT_TAKEN == a_parVars->m_partition[index])
      {
        ...
        ELEMTYPE growth0 = CalcRectVolume(&rect0) - a_parVars->m_area[0];
        ELEMTYPE growth1 = CalcRectVolume(&rect1) - a_parVars->m_area[1];
