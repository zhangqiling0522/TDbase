...
int main(int argc, char **argv){
	TMesh *poly = new TMesh();
	//poly->load(argv[1]);
	string str = tdbase::read_file(argv[1]);
	poly->parse(str);
	vector<TMesh *> ind = poly->depart();
	char path[256];
	for(int i=0;i<ind.size();i++){
		if(ind[i]->vertices.size()>100){
			ind[i]->evaluate();
			...
			//ind[i]->fill_holes();
			if(argc>2){
				sprintf(path,"%s/fixed_%ld_%ld.OFF",argv[2],ind[i]->vertices.size(),ind[i]->faces.size());
			}else{
				sprintf(path,"/gisdata/fixed_%ld_%ld.OFF",ind[i]->vertices.size(),ind[i]->faces.size());
			}
			ind[i]->dumpto(path);
		}
		delete ind[i];
